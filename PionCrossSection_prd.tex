...
Even if pions are by far the biggest beam component in selected beamline candidates, residual muons and electrons are present in the sample. We estimate the muon and electron contribution to the incident and interacting flux via MC. Our goal is to estimate  $B^{ \text{Int}}_i$ and  $B^{ \text{Inc}}_i$ , i.e. the number of non-pion entries in the interacting and incident flux at the kinetic energy bin $E^{kin}_i$, respectively. The estimated percentages of muons, $w_\mu$,  and electrons, $w_e$,  in the sample of pion beamline candidates are reported in Table \ref{tab:MCafterCutContaminants}. Thus, we estimate 
$B^{ \text{Int}}_i$ to be
\begin{equation}
B^{ \text{Int}}_i = M^{ \text{Int}}_i + E^{ \text{Int}}_i =  \frac{w_\mu P^{Data}}{P^{MC}} M^{MC}_{ \text{Int,i}} + E^{ \text{Int}}_i
\end{equation}

where $M^{ \text{Int}}_i$ and $E^{ \text{Int}}_i$ are the estimated muon and electron contribution, $P^{Data}$ the total number of beamline pion candidates in data .
We estimate the percentage of muons and electrons at the TPC front face is shown in Table \ref{tab:beamline}.  



We simulate the muon and electron component in the TPC, we apply the tracking algorithm and obtain the interacting and incident fluxes for the electrons and muons in each. 

We simulate the muons and electrons in the TPC evaluate their contribution to the cross section. We scale their contribution to the interacting and incident flux to the number of . 
