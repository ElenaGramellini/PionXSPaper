...
Most historic measurements of pion hadronic cross sections are performed on thin targets [\textcolor{red}{cit experiments}]. At their core, these experiments consists in shooting a beam of particles with a known flux on a thin slab of material and recording the outgoing flux. Our approach to the cross section measurement is complementary: we record the incident and the interacting flux, since our target (the LArTPC) coincides with the sensitive volume.  


\section{\label{sec:ExperimentalSetup}Experimental Setup}
To its bare bones, the LArIAT experiments consist in a LArTPC deployed on a beam of charge particles at the Fermilab FTBF facility. The experimental setup changed significantly during LArIAT's three seasons of data taking regarding both the beamline and TPC components. We acquired the data used in this work  during the 24-weeks of data taking of the second season, which we refers as to Run II.  

We briefly describe here only the features and use of the LArIAT Run II beamline detectors and LArTPC  relevant to the cross section measurement; we refer the reader to \cite{LArIATDet} for a detailed and complete description of LArIAT's experimental setup.
...
Scope of the LArIAT beamline instrumentation is to identify the particle type and measure the momentum of the beam's particles entering the LArTPC. Figure \ref{fig:beamlinebird} shows a bird's eye view of the LArIAT tertiary beamline where we highlighted the key components for the cross section measurement:  two bending electromagnets (yellow), a set of four wire chambers (red) and two time-of-flight scintillating paddles (blue).  The magnets' polarity determines the charge of the particles selected in the tertiary beam: running the magnets in negative polarity selects negatively charged particles. The strength for the magnetic field is set by the amplitude of the current in the magnets. For this work,  the current settings employed were -60A %(B $\sim$0.21 T) 
...
\begin{equation}
\begin{split}
 E_{j}^{kin}  & = E^{kin}_{Front Face} -  \sum_{n < j} E_{\text{Dep},n}.
\end{split}
\label{eq:KEj}
\end{equation}
