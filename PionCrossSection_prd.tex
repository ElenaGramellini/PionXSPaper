...
We rely on the capacity of the tracking algorithm to identify the incoming pion in the TPC and to stop the tracking at the interaction point, evaluating whether or not the pion interacted.  Thus, the interacting flux depends on the efficiency of the tracking of finding the pion in the TPC and of finding the interaction point. It should be noted that the incident flux also depends on the same tracking efficiencies, albeit in a different way; if a scattering is missed (inefficiency for the interaction flux), several extra slices are counted for the incident flux (background for the incident flux); if the tracking stops before the interaction point (background for the interaction flux), several slices are missing the incident flux (inefficiency for the incident flux).





Among other quantities such as the track length, the tracking efficiency is a function of the scattering angle; we evaluate 
